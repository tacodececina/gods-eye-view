...
// ---------------------------------------------------------------------------
async function main() {
  // The deterministic terrain transport also uses the real geoid from Node.
  ...
  try {
    ...
    page.on('request', (request) => {
      ...
      // Deterministic terrain fixture for the public static distribution.
      // Production intentionally has no /api backend. Preserve all corridor,
      // datum and floor assertions below: only supply their upstream input.
      // 160 m orthometric approximates the Austin cohort; the real bundled
      // geoid conversion remains in the application and is NOT stubbed.
      if (
        ...
        request.respond({
          ...
          body: JSON.stringify({
            results: points.map((point) => {
              ...
              return { ellipsoid: 160 + geoidHeight(lat, lon) };
            }),
          }),
        });
        return;
    ...
    if (!runnerReady) {
      ...
      const panelVsSpoken = await evalPage(async () => {
        // EYEINSKY visibly restricts provider controls in its public build.
        // This harness has explicitly supplied synthetic provider transports;
        // enable ONLY their context group for the existing interaction tests.
        // The default public restriction is independently checked by the
        // surface/preview journeys; no test below is skipped or weakened.
        const fixtureControls = document.querySelector(
          '#eye-context-host > fieldset',
        );
        if (fixtureControls) fixtureControls.disabled = false;
        const manager = window.__godsEyeView.dataManager;
