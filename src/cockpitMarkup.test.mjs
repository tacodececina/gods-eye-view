...
test('Cockpit has one reset action beside its bottom exit path', () => {
  ...
  assert.match(
    html,
    /id="map-view-switch"[^>]*aria-label="Exit cockpit view"[^>]*aria-keyshortcuts="Escape C"[\s\S]*?close_fullscreen[\s\S]*?SALIR DE CABINA/,
  );
  const topCenterActions = html.match(
    /<nav class="eye-instruments"[\s\S]*?<\/nav>/,
  );
  assert.ok(topCenterActions, 'EYEINSKY globe instruments are missing');
  assert.match(
    topCenterActions[0],
    /id="eye-zoom-in"[\s\S]*?id="eye-zoom-out"[\s\S]*?id="eye-home"[\s\S]*?id="eye-north"[\s\S]*?id="eye-grid"/,
  );
  assert.equal(
  ...
  assert.equal(
    (html.match(/id="eye-home"/g) || []).length,
    1,
  ...
  assert.match(
    viewSwitcher[0],
    /id="cockpit-reset-globe"[^>]*type="button"[^>]*aria-label="Reset cockpit to full globe view"[^>]*hidden[\s\S]*?public[\s\S]*?RESTABLECER[\s\S]*?id="map-view-switch"/,
  );
...
test('Cockpit vision cycle exposes exactly five real visual styles without NONE', () => {
  ...
  assert.match(
    html,
    /id="cockpit-vision-current-label"[^>]*>NATURAL<\/strong>/,
  );
  assert.match(
    ui,
...
test('Cockpit owns a focused shared Display portal and compact Radio controls', () => {
  ...
  assert.match(
    html,
    /id="clear-selected-layers"[^>]*aria-label="Apagar las capas seleccionadas"/,
  );
  assert.match(html, /id="eye-home"[^>]*aria-label="Vista global"/);
  assert.match(
    css,
...
test('Global Context standby describes both chooser modes', () => {
  ...
  assert.match(match[0], /CONTACTOS — aeronaves · barcos · lugares/);
  assert.match(match[0], /MISIONES — lanzamientos y órbitas/);
});
