...
test('the standalone document expands every component once and preserves unique element ids', () => {
  ...
  const selected = [...source.matchAll(/gev:template ([\w-]+)/g)].map(
    (m) => m[1],
  );
  assert.deepEqual(
    [...selected].sort(),
    APPLICATION_TEMPLATES.filter((name) => name !== 'welcome').sort(),
  );
  assert.doesNotMatch(html, /id="first-run-launcher"/);
  assert.match(html, /id="key-setup"/);
