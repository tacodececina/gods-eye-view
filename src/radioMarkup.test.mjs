...
test('Radio is nested inside Context with separate disclosure and power controls', () => {
  ...
  // Match the actual rail boundary; nested asides and formatter indentation
  // must not masquerade as the closing tag of the parent rail.
  const railStart = html.indexOf('<aside id="right-context-rail"');
  let depth = 0,
    contextEnd = -1;
  for (const match of html.slice(railStart).matchAll(/<\/?aside\b[^>]*>/g)) {
    depth += match[0].startsWith('</') ? -1 : 1;
    if (depth === 0) {
      contextEnd = railStart + match.index;
      break;
    }
  }
  assert.ok(railStart >= 0 && railStart < contextStart);
  assert.ok(
  ...
  assert.match(html, /id="radio-tuner-band-label">DIRECTORIO/);
  assert.match(html, /id="radio-tuner-slider"[^>]*type="range"/);
  assert.match(html, /id="radio-tuner-needle"[^>]*aria-hidden="true"/);
  assert.match(html, /AJUSTE A EMISORAS DISPONIBLES/);
  assert.match(html, /class="radio-tuner-scale" aria-hidden="true"><\/div>/);
  assert.match(html, /DIRECTORIO: RADIO BROWSER/);
  assert.match(
    html,
    /El audio se conecta a la emisora sólo al pulsar reproducir/,
  );
